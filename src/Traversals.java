...
public class Traversals {
  ...
  public static int countDistinctValues(TreeNode<Integer> node) {
    ...
    countDistinct(node, visited);
    return visited.size();

  }
  private static void countDistinct(TreeNode<Integer> node, Set<Integer> visited) {
    if (node == null) {
      return;
    }
    visited.add(node.value);
    countDistinct(node.left, visited);
    countDistinct(node.right, visited);
  }
  ...
  public static boolean hasStrictlyIncreasingPath(TreeNode<Integer> node) {
    ...
    return hasIncreasing(node, null);
  }

  private static boolean hasIncreasing(TreeNode<Integer> node, Integer prev) {
    if (node == null) return false;
    ...
    return hasIncreasing(node.left, node.value) || hasIncreasing(node.right, node.value);
  }
